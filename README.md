...
* Select the [branch](https://github.com/Shopkeepers/Language-Files/branches) that matches your version of the Shopkeepers plugin: 
  * Either select the branch in the drop-down menu at the top-left of this page, or click the link above for an overview of all branches and then click a branch there to select it. More information on how to view all branches can also be found inside the [GitHub Docs](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/managing-branches-in-your-repository/viewing-branches-in-your-repository).
  * Some versions of the Shopkeepers plugin had no changes to their language files, so there is no separate branch for them. In that case, simply pick the next lower version for which this repository contains a branch.
...
* [Create a `Pull Request`](https://help.github.com/en/articles/creating-a-pull-request-from-a-fork): This creates a request to apply the changes from your fork to this base repository.
  ...
  * GitHub might have created a new 'branch' for your changes (more on those below). Be sure to select both the correct `base` branch (the branch inside this base repository you want to apply your changes to), and the correct `compare` branch (the branch inside your fork repository that contains your changes).
...
* The old language file inside the branch of the previous version has to stay intact: Do not edit the old language file, but instead create a new language file with the same name inside the branch for the new plugin version.
...
* Go through the list of changes inside the [`CHANGELOG`](CHANGELOG.md) file and step-by-step apply these changes to the newly added language file.  
The changelog will often times only mention that some message has changed. In order to figure out what exactly has changed, you will need to compare the contents of the default (`en-default`) language file of the previous plugin version with the contents of the default language file of the new plugin version. This can be achieved in the following ways:
  * Use some text comparison tool like https://text-compare.com/.
  * Select the default `en-default` language file inside the branch of the latest plugin version, and then click the `[History]` button in the top-right corner of the page. This will bring up the commit history of this file. Select the commit for the latest plugin version update: This will visualize all the lines that were added, removed, or have changed compared to the previous version of this file.
  * Open the compare page [`https://github.com/Shopkeepers/Language-Files/compare`](https://github.com/Shopkeepers/Language-Files/compare) and select the two branches (i.e. the two plugin versions) you want to compare. At the bottom of the comparison page, GitHub lists all files that have changed, and all the lines that have changed within each of these files. Search for the default `en-default` language file to inspect its changes.
